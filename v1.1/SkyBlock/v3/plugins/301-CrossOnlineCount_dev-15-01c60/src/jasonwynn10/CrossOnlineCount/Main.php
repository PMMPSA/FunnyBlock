...
use jasonwynn10\CrossOnlineCount\libs\libpmquery\PMQuery;
use jasonwynn10\CrossOnlineCount\libs\libpmquery\PmQueryException;

use jasonwynn10\CrossOnlineCount\libs\spoondetector\SpoonDetector;
			...
			foreach($level->getEntities() as $entity) {
				if(isset($entity->namedtag->server)) {
					...
					try {
						...
						$lines = explode("\n", $entity->getNameTag());
						$lines[0] = TextFormat::GREEN.$online." người chơi".TextFormat::WHITE;
						$nametag = implode("\n", $lines);
						$entity->setNameTag($nametag);
						...
						$lines = explode("\n", $entity->getNameTag());
						$lines[0] = TextFormat::RED."Máy chủ ngoại tuyến".TextFormat::WHITE;
						$nametag = implode("\n", $lines);
						$entity->setNameTag($nametag);
					}
				}
