...
class SkyBlockStructure extends Populator{
	...
	public static function placeObject(ChunkManager $level, $chunk, $Xofchunk, $Zofchunk){
		...
		$level->setBlockIdAt($vec->x + 8, 69, $vec->z + 7, Block::HAY_BALE);
		$level->setBlockIdAt($vec->x + 8, 70, $vec->z + 7, Block::HAY_BALE);
		$level->setBlockIdAt($vec->x + 7, 69, $vec->z + 8, Block::HAY_BALE);
		$level->setBlockIdAt($vec->x + 10, 69, $vec->z + 9, Block::FENCE);
		$level->setBlockIdAt($vec->x + 10, 69, $vec->z + 8, Block::FENCE);
		$level->setBlockIdAt($vec->x + 9, 69, $vec->z + 10, Block::FENCE);
		$level->setBlockIdAt($vec->x + 8, 69, $vec->z + 10, Block::FENCE);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 9, Block::FARMLAND);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 8, Block::FARMLAND);
		$level->setBlockIdAt($vec->x + 8, 68, $vec->z + 9, Block::FARMLAND);
		$level->setBlockIdAt($vec->x + 9, 69, $vec->z + 9, Block::WHEAT_BLOCK);
		$level->setBlockIdAt($vec->x + 9, 69, $vec->z + 8, Block::WHEAT_BLOCK);
		$level->setBlockIdAt($vec->x + 8, 69, $vec->z + 9, Block::WHEAT_BLOCK);
